...
class Spider(Spider):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        try:
            ...
            return {'list': videos, 'page': int(pg) if pg else 1}
        except Exception as e:
            print(f"categoryContent 失败: {str(e)}")
            return {'list': [], 'page': 1}
    ...
    def searchContent(self, key, quick, pg='1'):
        try:
            ...
            print(f"searchContent 失败: {str(e)}")
            return {'list': [], 'page': 1}
    ...
    def detailContent(self, ids):
        try:
            ...
            for i in data.get('episodeList', []):
                play_url.append(f"{i['episode']}${vid}@{currentplayerid}@{i['id']}@episode")
            if play_url:
            ...
            for i2 in moviePlayerList:
                if i2.get('id') == currentplayerid:
                    show.append(i2.get('moviePlayerName', '默认线路'))
            ...
            for j in moviePlayerList:
                ...
                for k in range(1, episodeTotal + 1):
                    play_url.append(f"第{k}集${k}@{playerid}@{vid}@virtual")
                if play_url:
                    play_urls.append('#'.join(play_url))
                    if j.get('moviePlayerName') and j.get('moviePlayerName') not in show:
            ...
            video = {
                ...
                'vod_play_from': '$$$'.join(show) if show else '默认线路',
                'vod_play_url': '$$$'.join(play_urls) if play_urls else ''
            }
            return {'list': [video]}
        except Exception as e:
            print(f"detailContent 失败: {str(e)}")
            return {'list': []}
    ...
    def playerContent(self, flag, id, vipflags):
        try:
            ...
            print(f"playerContent 失败: {str(e)}")
